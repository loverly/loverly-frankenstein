...
AbstractTable.prototype.list = function (params, options, callback) {
  ...
  query.order = options.sortField + ' ' + options.sortOrder;
...
AbstractTable.prototype.buildQuery = function (params, options) {
  ...
  var alias = (options.alias ? options.alias : 'p') + '.';
  ...
  if (params) {
    ...
    for (var i in params) {
      ...
      if (valType === '[object String]'){
        ...
        whereClause += alias + i + ' IN (' + val.join() + ')';
      }
      else {
        whereClause += alias + i + ' = ' + val;
      }
  ...
  if (options.sortField && options.sortOrder) {
    query += ' ORDER BY ' + options.sortField + ' ' + options.sortOrder;
  }
