...
Instance.prototype.create = function (callback) {
  ...
  var dependentSubmodels = this.getDependentSubmodelsToCreate();
 ...
 */
Instance.prototype.getDependentSubmodelsToCreate = function () {
  ...
  for (i in fieldMetainfo) {
    if (fieldMetainfo[i].type === 'submodel_array') {
  ...
  for (i in submodelMetainfo) {
    ...
    dependentSubmodels.push({foreign_key: fk, instances: instances});
  }
