 ...
 * @constructor
 */
var Instance = function (modelName, definition, mapping, dataSources, validator, logger) {
  var self = this;
  var meta;
  ...
  // Inject the validator via property injection
  meta.validator = null;
  meta.check = null;
...
Instance.prototype.recursiveDeepCopy = function (source, target, definition, view, prefix, fields, includeAll) {
  ...
  for (i in source) {
    ...
    shouldInclude = includeAll || (fields[prefix + i]) ? true : false;
...
/**
 * Provide the ability to revert all changed fields to their previous value
 *
 */
Instance.prototype.revertChanges = function () {
...
Instance.prototype.isSubDocumentDefinition = function (obj) {
  ...
  for (i in obj) {
    if (typeof obj[i] === 'object' && i !== 'constraints' && i !== 'views') {
      return true;
    }
  }
...
Instance.prototype.doDocumentFieldsHaveError = function (definition, metadata, data) {
  ...
  for (i in definition) {
    ...
    if (!field || field === '') {
      if (definition[i].required && !definition[i].defaultValue) {
        hasError = true;
        metadata[i].hasError = true;
...
Instance.prototype.bindDataByDefinition = function (definition, metadata, target, data, resetAllFields, isUpdate) {
  ...
  for (i in definition) {
    ...
    if (definition[i] instanceof Array && this.__meta.mapping.fields[i].type === 'submodel_array') {
      ...
      for (j in data[i]) {
        ...
        tempInst.bind(data[i][j], isUpdate);
        target[i][j] = tempInst;
      }
...
Instance.prototype.bindOrmInstanceToObject = function (definition, mapPrefix, fieldMappings, instance, table, ormInstance) {
  ...
  for (i in definition) {
    ...
    data = (mapping && mapping.alias) ? ormInstance[mapping.alias] : ormInstance[i];
...
Instance.prototype.create = function (callback) {
  ...
  this.createPrimaryRecord(primaryInstance, primaryTable, dependents, callback);
};

/**
 * Start by saving the primary then updating the foreign key on each of the
 * dependent records.
 *
 ...
 * @param callback
 */
Instance.prototype.createPrimaryRecord = function (primaryInstance, primaryTable, dependents, callback) {
  this.setSourceOpCallback(callback);
  primaryInstance
    .save()
    .error(this.failSourceOp)
    .success(this.generateDependentRecordsClosure(primaryTable, dependents, callback));
};
 ...
 * @returns {Function}
 */
Instance.prototype.generateDependentRecordsClosure = function (primaryTable, dependents, callback) {
  var self = this;

  return function (primaryInstance) {
    var i, instance, info, table;

    self.bindOrmInstance(primaryTable, primaryInstance);
    self.setSourceOpCallback(callback);

    for (i in dependents) {
      instance = dependents[i].instance;
      info = dependents[i].info;
      table = dependents[i].table;
...
Instance.prototype.getUpdatedFields = function (fieldMetadata, definition, data) {
  ...
  for (i in definition) {
    if (this.isSubDocumentDefinition(definition[i])) {
      updatedFields[i] = this.getUpdatedFields(
        fieldMetadata[i],
        definition[i],
        data[i]
      );
...
Instance.prototype.mapFieldUpdatesToInstances = function (data, definition, prefix, fieldMapping, result) {
  ...
  for (i in data) {
    ...
    } else if (mapping.type === 'submodel_array') {
