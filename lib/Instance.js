...
Instance.prototype.recursivelyGetFieldErrors = function (definition, metadata, data, fieldErrors) {
  ...
  for (i in definition) {
    ...
    if (this.isSubDocumentDefinition(definition[i])) {
      ...
      this.recursivelyGetFieldErrors(definition[i], metadata[i], data[i], fieldErrors[i].childErrors);
...
Instance.prototype.recursiveDeepCopy = function (source, target, definition, view, prefix, fields, includeAll) {
  ...
  for (i in source) {
    ...
    } else if (definition[i].type === "DATE" && source[i] instanceof Date) {
      target[i] = source[i].getFullYear() + '-' + source[i].getMonth() + '-' + source[i].getDate();
      didUpdate = true;
    } else {
      target[i] = source[i];
      didUpdate = true;
