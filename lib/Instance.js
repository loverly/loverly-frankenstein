...
Instance.prototype.isSubDocumentDefinition = function (obj) {
  ...
  for (i in obj) {
    if (typeof obj[i] === 'object' && !ignoredKeys[i]) {
      return true;
    }
  }
...
Instance.prototype.getView = function () {
  return this.__meta.view;
};
...
Instance.prototype.doDocumentFieldsHaveError = function (definition, metadata, data) {
  ...
  for (i in definition) {
    ...
    if (typeof data === 'object' && data !== null) {
      ...
      console.log("Field was null:", field)
      field = null;
    }
...
Instance.prototype.bindSourceInstanceToSelf = function (definitions, target, source, instance) {
  ...
  for (i in definitions) {
    ...
    target[i] = (typeof data !== 'undefined') ? data : target[i];
 ...
 * @param result
 */
Instance.prototype.mapFieldsToSourceInstances = function (data, definition, sources, instances, topLevelKey, keepTopLevel) {
  ...
  for (i in data) {
    ...
    if (this.isSubDocumentDefinition(definition[i])) {
      ...
      //build the nested path if nested level
      var path = topLevelKey ? topLevelKey + "." + i : i;
    ...
    if (!mapping.type || mapping.type === MAPPING_TYPES.FIELD) {
      ...
      if (typeof instance.set === 'function') {
        ...
        //build nested path when in subdocument
        if (keepTopLevel) {
          key = topLevelKey + "." + key
        }
      ...
      // For mongoose.js - you have to tell it that objects or mixed types
      // should be updated
      if (definition[i].type === 'OBJECT' && instance.markModified) {
