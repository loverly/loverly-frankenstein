...
Instance.prototype.recursiveDeepCopy = function (source, target, definition, view, prefix, fields, includeAll) {
  ...
  for (i in source) {
    ...
    shouldInclude = (includeAll || definition[i].views[view] || fields[prefix + i]);
...
Instance.prototype.bindFromSourceInstance = function (source, instance) {
  ...
  // delete
  meta.instances[source.name] = instance;
...
Instance.prototype.bindSourceInstanceToSelf = function (definition, target, source, instance) {
  ...
  for (i in definition) {
    ...
    target[i] = (data && mapping.deserialize) ? mapping.deserialize(data) : data;
  }
...
Instance.prototype.create = function (callback) {
  ...
  // Map the updated fields to their data sources
  for (i in fieldsToSave) {
  ...
  var instances = {
    primary: primarySource.createInstance()
  };
  ...
  var primaryInstance = instances['primary'];
  ...
  primaryInstance.create().done(saveDependentSourcesCallback);
...
Instance.prototype.mapFieldsToSourceInstances = function (data, definition, sources, instances) {
  ...
  for (i in data) {
    ...
    if (!mapping.type || mapping.type === MAPPING_TYPES.FIELD) {
      ...
      if (!instanceWrapper) {
        instanceWrapper = {
          instance: sources[sourceName].createInstance(),
