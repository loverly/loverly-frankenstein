  ...
  for (i in definition) {
    ...
    if (typeof data === 'object') {
      ...
      if ((typeof field === 'undefined' || field === null) && typeof defaultValue !== 'undefined') {
        ...
        data[i] = (typeof defaultValue === 'function') ? defaultValue() : defaultValue;
      }
    } else {
      field = null;
    }
    ...
    for (j in constraints) {
      ...
      // New-style constraint definitions
      ...
      if (!errorMessage) {
        errorMessage = modelErrorMessages[constraint] || 'NO MESSAGE: ' + constraint;
      }
...
Instance.prototype.recursivelyGetFieldErrors = function (definition, metadata, data, fieldErrors) {
  ...
  for (i in definition) {
    ...
    } else if (definition[i] instanceof Array || definition[i].type === MAPPING_TYPES.SUBMODEL_ARRAY) {
      fieldErrors[i] = {error: metadata[i].lastError, childErrors: {}};
      for (j in data[i]) {
        fieldErrors[i].childErrors[j] = data[i][j].getErrors();
      }
...
Instance.prototype.bind = function (data, isUpdate) {
  ...
  if (meta.isNextGenModel) {
    ...
    this.bindDataByDefinition_deprecated(
      this.__meta.definition,
      this.__meta.fields,
      this,
      data,
 ...
 * @param isUpdate
 */
Instance.prototype.bindDataByDefinition_deprecated = function (definition, metadata, target, data, resetAllFields, isUpdate) {
  var i, j, submodel, tempInst;
  var isSubmodelArrayInOldParadigm;
  ...
  for (i in definition) {
    ...
    isSubmodelArrayInOldParadigm = (
      definition[i] instanceof Array &&
      this.__meta.mapping.fields[i].type === MAPPING_TYPES.SUBMODEL_ARRAY
    );

    if (isSubmodelArrayInOldParadigm) {
      if (!target[i]) {
        target[i] = [];
      }
    ...
    for (i in instances) {
      ...
      if (!instance) {
        ...
        for (j in submodelArrayInstances) {
          ...
          instance.set(source.foreign_key, primaryInstance.id);

          submodelArrayInstances[j]
            .save()
...
Instance.prototype.update = function (callback) {
  ...
  if (!this.__meta.isNextGenModel) {
    this.update_deprecated(callback);
    return;
  }
  ...
  for (i in instances) {
    ...
    if (!instance) {
      ...
      for (j in submodelArrayInstances) {
        instance = submodelArrayInstances[j];
        instance.set(source.foreignKey, this.id);
        ...
        if (instance.shouldDelete()) {
          ...
          instance
            .save()
            .complete(self.handleSourceOpCompletion);
        }
      }
...
Instance.prototype.destroy = function (callback) {
  ...
  if (!meta.isNextGenModel) {
    this.destroy_deprecated(callback);
    return;
  }
 ...
 * @deprecated
 */
Instance.prototype.destroy_deprecated = function (callback) {
  var meta = this.__meta;
  var dbInfo = meta.mapping.sources.databases;
  var dbs = meta.instances.databases;
  var i, j, instance;
