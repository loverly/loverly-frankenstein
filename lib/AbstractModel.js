...
var AbstractModel = function () {
  ...
  // Control what view is used for updates - affects validation
  this.updateView = this.updateView || 'all';
...
AbstractModel.modifyDefinitions = function (fieldDefinitions) {
  ...
  for (i in fieldDefinitions) {
    ...
    filters = (definition.dataFilters) ? definition.dataFilters : views.slice();
    filterLookup = {};
    ...
    for (j in filters) {
      filterLookup[filters[j]] = true;
    }

    definition.dataFilters = filterLookup;
...
/**
 * A helper function to modify model options and merge them with the standard defaults
 * as well as model-specified defaults.
 *
 * @method prepareModelOptions
 */
AbstractModel.prototype.prepareModelOptions = function (params, rawOptions) {
 ...
 * TODO: Lots of duplicate code here - need to abstract it out
 *
 * @method count
 */
AbstractModel.prototype.count = function (params, rawOptions, callback) {
  var i;
  var options = this.prepareModelOptions(params, rawOptions);
  ...
  for (i in sources) {
    ...
    if (sourceInfo.is_primary) {
      source.count(query, options, callback);
      break;
    }
  }
};
 ...
 * @param callback
 */
AbstractModel.prototype.list = function (params, rawOptions, callback) {
  var i;
  var options = this.prepareModelOptions(params, rawOptions);
  ...
  for (i in sources) {
    ...
    // Create a query object per source based on the params and the named query
    // specified
    query = this.getQueryForSource(options.query, params, sourceInfo, options.check_query_params);
 ...
 *
 * @method getQueryForSource
 */
AbstractModel.prototype.getQueryForSource = function (queryName, customParams, sourceInfo, isStrict) {
  var i;
  var query = {};
  ...
  if (type === this.SOURCE_MAPPING_TYPES.ONE_TO_ONE || type === this.SOURCE_MAPPING_TYPES.ONE_TO_ONE_REF) {
    ...
    for (i in query) {
      if (isStrict) {
        query[i] = this.cleanQueryParam(i, query[i]);
      }
  ...
  if (finalQuery.hasOwnProperty('null')) {
    delete finalQuery.null;
  }
...
/**
 * Compares the definition type to the parameter being passed in and tries to
 * coerce the type or scrub it if we are in strict mode.
 *
 * @method cleanQueryParam
 */
AbstractModel.prototype.cleanQueryParam = function (key, value) {
  var definition = AbstractModel.getChildWithDotNotation(key, this.definition);
  var cleaned;
  ...
  if (!key || key === 'null') {
    ...
    if (key.indexOf('.') === -1) {
      console.warn('Error in', this.name, 'could not find definition for query param', key);
    }

    cleaned = value;
  } else if ((definition.type === 'NUMBER' || definition.type === 'INTEGER' || definition.type === 'STRING') && typeof value === 'object') {
    cleaned = [];
    for (var i in value) {
      cleaned.push(this.coerceQueryParamType(definition.type, key, value[i]));
    }
  } else {
    cleaned = this.coerceQueryParamType(definition.type, key, value);
  }
...
/**
 * Separate out the actual coercion to types into a separate function so it can
 * be applied multiple times on a single param in the case that it's an array.
 *
 * @param type
 * @param key
 * @param value
 * @returns {*}
 */
AbstractModel.prototype.coerceQueryParamType = function (type, key, value) {
  if (type === 'STRING') {
    ...
    if (typeof value === 'string') {
      if (/[0-9.]+/.test(value)) {
        ...
        console.warn('Error in', this.name, 'invalid number for key', key, 'with value', value);
        value = null;
      }
...
AbstractModel.prototype.transformQueryParamKeyToAlias = function (key, sourceName, isPrimary) {
  ...
  if (!mapping || !mapping.source) {
    ...
    //TODO this is not working with the V3 Models. Using the version without check for submodels
    //return (isPrimary && key.indexOf('.') === -1) ? key : false;

    return (isPrimary) ? key : false;
  }
...
AbstractModel.prototype.generateListFromSourceCallback = function (source, meta, options, callback) {
  ...
  return function (err, instances) {
    ...
    for (i in instances) {
      ...
      // Set the view for binding purposes
      newInstance.setView(options.view);
      ...
      for (var j in data) {
        if (instances[i].get(localKey) === data[j].get(foreignKey)) {
          if (type === ONE_TO_MANY) {
            ...
            obj = (typeof data[j].toObject === 'function') ? data[j].toObject() : data[j];
...
AbstractModel.prototype.read = function (params, options, callback) {
  ...
  this.list(params, opts, function (err, list) {
    if (err) {
      ...
      callback({code: 404, msg: "Not Found", error: "Not Found"});
    } else {
...
AbstractModel.prototype.shouldReadFromSource = function (source, view, includedFields, definitions, prefix, includeAll) {
  ...
  for (i in definitions) {
    ...
    mapping = definitions[i].mapping;
    fieldFilters = definitions[i].dataFilters;
    ...
    if (fieldFilters[view] || (includedFields && includedFields[prefix + i]) || includeAll) {
      return true;
    }
  }
...
AbstractModel.prototype.update = function (params, data, callback) {
  this.read(params, {view: this.updateView, should_resolve_references: false}, function (err, instance) {
    if (err || !instance) {
      callback(err);
      return;
    }
...
AbstractModel.prototype['delete'] = function (params, callback) {
  this.read(params, {view: 'delete'}, function (err, instance) {
    if (err || !instance) {
      callback(err);
      return;
    }
...
AbstractModel.prototype.search = function (params, options, callback) {
  ...
  for (i in this.sources) {
    ...
    if (source.relationship === this.SOURCE_MAPPING_TYPES.SEARCH) {
      if (!options.search_source || options.search_source === i) {
        searchSource = source;
      }
    }
  }
