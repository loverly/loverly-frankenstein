...
AbstractModel.prototype.addModelReferenceDefinitions = function (definitions) {
  ...
  for (var i in definitions) {
    ...
    // HACK - disables this hack for when the reference is not included within
    //        the primary source via a join.
    if (definition.mapping.isRefNotIncludedInPrimarySource) {
      continue;
    }
...
AbstractModel.prototype.list = function (params, options, callback) {
  ...
  for (i in sources) {
    ...
    // specified
    query = this.getQueryForSource(options.query, params, source.name, sourceInfo.is_primary);
 ...
 * @method method
 */
AbstractModel.prototype.getQueryForSource = function (queryName, customParams, sourceName, isPrimary) {
  var i;
  var query = {};
...
AbstractModel.prototype.generateListFromSourceCallback = function (source, meta, options, callback) {
  ...
  return function (err, instances) {
    ...
    self.decorateList(list, options, callback);
  };
};
 ...
 * @method method
 */
AbstractModel.generateDecoratorCallback = function (status, isOneToMany, instances, localKey, foreignKey, field, callback) {
  status.cnt++;
    ...
    for (var i in instances) {
      for (var j in data) {
        if (instances[i].get(localKey) === data[j].get(foreignKey)) {
          if (isOneToMany) {
            if (!(instances[i][field] instanceof Array)) {
              instances[i][field] = [];
            }

            instances[i][field].push(data[j]);
          } else {
            instances[i][field] = data[j].toObject('all');
          }
        }
      }
    }
 ...
 * @method pluck
 */
var pluck = function (field, array) {
  var values = [];
  for (var i in array) {
...
AbstractModel.prototype.decorateList = function (instances, options, callback) {
  ...
  for (i in sources) {
    ...
    // specified
    query = {};
    query = this.getQueryForSource(opts.query, query, source.name);

    // Get the list of local keys to include
    query.parent_id = pluck(localKey, instances);
    ...
    source.list(
      query,
      opts,
      AbstractModel.generateDecoratorCallback(status, isOneToMany, instances, localKey, foreignKey, field, callback)
    );
  }
...
AbstractModel.prototype.read = function (params, options, callback) {
  ...
  var view = options.view;
  var fields = options.fields;
  options.should_resolve_references = true;
...
AbstractModel.prototype.generateSourceReadClosure = function (sourceInfo, newInst) {
  ...
  return function (err, instance) {
    ...
    // Should not return an array for a single read
...
AbstractModel.prototype.generateSubmodelReferenceClosure = function (field, newInst) {
  return function (err, refInstance) {
    ...
    // new instance
    var data = refInstance ? refInstance.toObject('all') : null;
