...
AbstractModel.prototype.copyReferenceDefinitionToModelDefinition = function (originalDef, reference) {
  var refdef;
  ...
  for (var i in reference) {
    ...
    if (AbstractModel.isSubDocumentDefinition(refdef)) {
      definition[i] = this.copyReferenceDefinitionToModelDefinition(originalDef, refdef);
      continue;
    }
  ...
  for (i in sources) {
    ...
    if (sourceInfo.is_primary) {
      if (options.should_include_meta) {
        source.count(params, options, this.generateGetListDataCallback(params, options, source, callback));
      } else {
        source.list(params, options, this.generateListFromSourceCallback(source, 0, options, callback));
      }
