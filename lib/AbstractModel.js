...
AbstractModel.prototype.read = function (params, options, callback) {
  ...
  for (i in dbMappings) {
    ...
    for (j in tableMappings) {
      ...
      if (!this.shouldReadFromSource(table, view, fields)) {
        continue;
      }
 ...
 * @returns {boolean}
 */
AbstractModel.prototype.shouldReadFromSource = function (source, view, fields) {
  if (view === 'all') {
    return true;
  }
  ...
  for (i in fields) {
    fieldLookup[fields[i]] = true;
  }

  var mapping, definition, views;
  ...
  for (i in fieldMappings) {
    ...
    // The fields was specifically requested
    if (fieldLookup[i]) {
      return true;
    }
    ...
    // definition
    definition = this.getChildWithDotNotation(i, definitions);
