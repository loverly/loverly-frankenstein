...
var AbstractModel = function () {
   ...
   *
   * @property MAPPING_TYPES.DB_FIELD
   * @type string
   */
  this.MAPPING_TYPES.DB_FIELD =  "db_field";

  /**
   * A calculated or derived field.
   *
   * @property MAPPING_TYPES.VIRTUAL_FIELD
   * @type string
   */
  this.MAPPING_TYPES.VIRTUAL_FIELD = "virtual_field";

  /**
   * A one-to-one mapping of fields from one model to another.
   *
   * TODO: Not sure what we'll use this for yet.
   *
   * @property MAPPING_TYPES.SUBMODEL_PROPERTY
   * @type string
   */
  this.MAPPING_TYPES.SUBMODEL_PROPERTY = "submodel_property";

  /**
   * A one-to-many relationship between this model and another sub-model.
   *
   * @property MAPPING_TYPES.SUBMODEL_ARRAY
   * @type string
   */
  this.MAPPING_TYPES.SUBMODEL_ARRAY =  "submodel_array";
   ...
   * link.
   *
   * @property MAPPING_TYPES.SUBMODEL_REFERENCE
   * @type string
   */
  this.MAPPING_TYPES.SUBMODEL_REFERENCE = "submodel_reference";

  /**
   * A property that belongs to an API object.  Not sure what we'll do with this
   * yet.
   *
   * @property MAPPING_TYPES.API_OBJECT
   * @type string
   */
  this.MAPPING_TYPES.API_OBJECT = "api_object";
...
AbstractModel.prototype.read = function (params, options, callback) {
  ...
  if (options.should_resolve_references) {
    newInstance.setSourceOpCallback(this.generateResolveReferencesCallback(callback));
  } else {
  ...
  var mappingFields;
  var mapping;
  var SUBMODEL_ARRAY = this.MAPPING_TYPES.SUBMODEL_ARRAY;
  var type;
...
AbstractModel.prototype.shouldReadFromDbSource = function (source, view, fields) {
  ...
  var i;
  var mapping;
  var DB_FIELD = this.MAPPING_TYPES.DB_FIELD;
...
AbstractModel.prototype.generateResolveReferencesCallback = function (options, callback) {
  ...
  return function (err, instance) {
    ...
    // Read from sub-models to populate one-to-many fields
    var definition;
    var definitions = self.definition;
    var modelMappings = self.mapping.sources.models;
    var model, targetField;
    var mappingFields;
    var mapping;
    var SUBMODEL_REFERENCE = self.MAPPING_TYPES.SUBMODEL_REFERENCE;
    var foreignKey;
    for (i in modelMappings) {
      model = this.sources.models[i].model;
      ...
      for (j in mappingFields) {
        ...
        if (mapping.type === SUBMODEL_REFERENCE) {
          targetField = j;
        }
      }
      ...
      // The field was not included in the resulting view and therefore we do not
      // need to query this model
      if (!self.isFieldIncludedInResult(targetField, view, fields)) {    
        continue;
      }

      instance.startSourceOp();
      model.read(
        {id: newInstance[targetField]}, 
        options, 
        this.generateSubmodelReferenceClosure(targetField, instance)
      );
    }
  };
};
