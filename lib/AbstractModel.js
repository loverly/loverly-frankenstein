...
AbstractModel.prototype.read = function (params, options, callback) {
  ...
  for (i in sources) {
    ...
    if (!this.shouldReadFromSource(source, view, includedFields)) {
      continue;
    }
    ...
    // This model is the parent of all of its sources, and therefore its id is
    // the parent_id
    query.parent_id = params.id;

    // Query the source appropriately depending on the relationship with this model
    if (source.relationship === ONE_TO_ONE) {
      source.read(
        query,
        options,
    ...
    } else if (source.relationship === ONE_TO_MANY) {
      source.list(
        query, 
        options, 
 ...
 * @returns {boolean}
 */
AbstractModel.prototype.shouldReadFromSource = function (source, view, includedFields) {
  var i;
  var mapping;
  var fieldViews;

  // The primary source is always included
  if (view === 'all' || source.isPrimary) {
    return true;
  }
  ...
  for (i in definitions) {
    ...
    if (fieldViews[view] || includedFields[i]) {
      return true;
    }
  }
