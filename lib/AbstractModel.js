...
var AbstractModel = function () {
  ...
  if (this.isNextGenModel) {
    ...
    // Set the primary source in an easy to reach variable
    for (i in this.sources) {
      if (this.sources[i].is_primary) {
        this.primarySource = this.sources[i];
      }
    }
  }
...
AbstractModel.transformDefinitionViewArrayToHash = function (fieldDefinitions) {
  ...
  for (i in fieldDefinitions) {
    ...
    if (AbstractModel.isSubDocumentDefinition(definition)) {
      transformDefinitionViewArrayToHash(definition);
      continue;
    }
...
AbstractModel.prototype.read = function (params, options, callback) {
  ...
  // Check if id has an alias
  var idMapping = this.definition.id.mapping;
  var idAlias = (idMapping && idMapping.alias) ? idMapping.alias : 'id'; 

  // Transform an array of fields to include in result into a lookup hash
  for (i in fields) {
    includedFields[fields[i]] = true;
  }
  ...
  for (i in sources) {
    ...
    if (!this.shouldReadFromSource(sourceInfo, view, includedFields)) {
      continue;
    }
    ...
    query[sourceInfo.foreign_key] = params.id;
    ...
    if (sourceInfo.relationship === ONE_TO_ONE) {
      source.read(
        query,
        options,
        this.generateSourceReadClosure(source, newInstance, options)
      );
      ...
      source.list(
        query, 
        options, 
        this.generateSourceListClosure(source, newInstance, options)
      );
 ...
 * @returns {boolean}
 */
AbstractModel.prototype.shouldReadFromSource = function (source, view, includedFields, definitions, prefix) {
  var i;
  var mapping;
  var fieldViews;
  ...
  for (i in definitions) {
    ...
    if (AbstractModel.isSubdocumentDefinition(definitions[i])) {
      if (this.shouldReadFromSource(source, view, includedFields, definitions[i], i + '.')) {
        return true;
      }
    ...
    if (mapping.source !== source.name) {
      continue;
    }

    // The field is either in the view or was explicitly included
    if (fieldViews[view] || includedFields[prefix + i]) {
      return true;
    }
  }
 ...
 * @returns {Function}
 */
AbstractModel.prototype.generateSourceReadClosure = function (source, newInst) {
  return function (err, instance) {
...
AbstractModel.prototype.update = function (params, data, callback) {
  this.read(params, {}, function (err, instance) {
    if (err || !instance) {
      callback(err);
      return;
    }
